...
class CPPKernel(Kernel):
    ...
    def compile_with_gpp(
        self, source_filename, binary_filename, cflags=None, ldflags=None
    ):
        cflags = [
            "-pedantic",
            "-fPIC",
    ...
    def _add_main(self, magics, code):
        ...
        # User input
        code = re.sub(
            r"cin>>|cin >>|cin  >>|std::cin>>|std::cin >>|std::cin  >>",
            r"std::cout<<GET_INPUT_STREAM_JP;std::cin >>",
            code,
        )
        code = re.sub(
            r"getline|std::getline|getline |std::getline ",
